...
export function ImpactSectionEditor({ sectionData, sectionIndex }: ImpactSectionEditorProps) {
      ...
      <div className="bg-gradient-to-r from-yellow-50 to-amber-50 px-4 py-3 border-b border-gray-100">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div>
              <h3 className="font-semibold text-gray-900">Impact Section</h3>
              <p className="text-xs text-gray-500">Showcase your organization's impact</p>
            </div>
          </div>
      ...
      {sectionData.enabled && !sectionData.collapsed && (
        <div className="p-4 space-y-4">
          <div className="grid gap-4">
            ...
            <ImageUploader 
              label="Impact Image"
              currentImageUrl={sectionData.props.impactImage}
              onUpload={(newUrl) => updateSectionField(sectionIndex, 'props.impactImage', newUrl)}
            />
          </div>
        </div>
      )}
